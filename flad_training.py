...
def  FederatedTrain(clients, model_type, outdir, time_window, max_flow_len, dataset_name, epochs='auto', steps='auto', training_mode = 'flad', weighted=False,optimizer='SGD',nr_experiments=EXPERIMENTS):
    ...
    while True:  # training epochs
        ...
        if stop == True:
            ...
            test_model(best_model, client_subset)
            break
...
def test_model(server_model, clients):
    f1_val_list = []
    print("Apply Test Data")

    for client in clients:
        X_test, Y_test = client['test']
        Y_pred = np.squeeze(server_model.predict(X_test, batch_size=2048) > 0.5)
        client_f1 = f1_score(Y_test, Y_pred)
        f1_val_list.append(client_f1)
        print(client['name'] + ": " + str(client_f1))
